object IA {
    ...
    def fonction_gain_attaque_bot(p : Character,k : Int,f_changement_joueur : Double, loop : Int) = {
        ...
        for (i <- 0 to 5) {
            ...
            var dmg_dealt = (((0.4 * p_pok.lvl.toDouble + 2.0) * atq.dmg.toDouble * p_pok.atk.toDouble / (50.0 * qi.defense.toDouble) + 3.0) * (atq.atype).affinites(qi.ptype))
            ...
            var multiplicateur_buff_def = (Func.mult_a(p_pok.defense_mult) / Func.mult_a(p_pok.defense_mult + atq_bd))
            ...
            var multiplicateur_buff_atk = (Func.mult_a(p_pok.atk_mult) / Func.mult_a(p_pok.atk_mult + atq_ba))
            ...
            var multiplicateur_buff_speed_useless = (Func.mult_a(p_pok.speed_mult) / Func.mult_a(p_pok.speed_mult + atq_bs))
            ...
            var multiplicateur_debuff_def = (Func.mult_a(qi.defense_mult) / Func.mult_a(qi.defense_mult - atq_dbd))
            var score_debuff_def = p_atq_dbd * scala.math.pow(quotient_vie_restante,2) * (multiplicateur_debuff_def - 1.0)

            //score associé au debuff d'attaque
            var (atq_dba,p_atq_dba) = atq.debuff_atk

            var multiplicateur_debuff_atk = (Func.mult_a(qi.atk_mult) / Func.mult_a(qi.atk_mult - atq_dba))
            var score_debuff_atk = p_atq_dba * scala.math.pow(quotient_vie_restante,2) * (multiplicateur_debuff_atk - 1.0)
